...
def main(page: ft.Page):
    ...
    def add_value(e):
        ...
        try:
            ...
            if chekbox_film.value == True:
                ...
                # db.commit()
                # print(movie_name.value, release_date.value,genre.value, state.value)
                ...
                # db.commit()
                # print(movie_name.value, release_date.value, state.value)
    ...
    def sqlite_get_all_data(name_table):
        ...
        if  chekbox_series.value == True:#director.value == "":
            for row in rows:
                lst.append(row)
                text_id.value = row[0]
        ...
        elif chekbox_anime.value == True:#aktors.disabled == True:
            for row in rows:
                lst.append(row)
                text_id.value = row[0]
                text_name.value = row[1]
        ...
        elif chekbox_mult.value ==True:#director.disabled  and aktors.disabled == True:
            for row in rows:
                lst.append(row)
                text_id.value = row[0]
                text_name.value = row[1]
        ...
        elif chekbox_mult_series.value == True:#director.disabled and aktors.disabled and genre.disabled == True:
            for row in rows:
                lst.append(row)
                text_id.value = row[0]
                text_name.value = row[1]
    ...
    def sql_test(name_table,name_column,value_column):
        ...
        rows = cur.fetchall()
        #print(rows,cur.fetchall(),cur.fetchone())

        if rows == [] :#or cur.fetchone() == None :#or page.on_event(page.error()):#page.on_event: page error Bad state: No element: #or rows == None or cur.fetchone() == None or cur.fetchall() == None or None:
        ...
        #     lv.controls.append(ft.Text(
        #         f"{text_id.value}   {text_name.value}   {text_data.value}    {text_genre.value}    {text_director.value}   {text_actors.value}   {text_state.value}"))
        #     #lv.controls.append(ft.Row([ft.Text(value=text_name.value),ft.Text(value=text_data.value),ft.Text(value=text_genre.value), ft.Text(value=text_director.value), ft.Text(value=text_actors.value),ft.Text(value=text_state.value,weight=ft.FontWeight.BOLD,color= ft.colors.GREEN)]))#ft.Text(f"{text_name.value}   {text_data.value}    {text_genre.value}    {text_director.value}   {text_actors.value}   {text_state.value}"))
        #             # count += 1
        #     page.update()
        #print(lst)
    ...
    movie_name = ft.TextField(label="Название", width=500,on_change= validate,input_filter=ft.InputFilter(allow= True, replacement_string="",regex_string=r"[0-9А-яA-Za-z,:.-\s/]+"))#,col={"sm": 6, "md": 4, "xl": 2})
    release_date = ft.TextField(label="год выхода", width=500,on_change= validate,input_filter=ft.InputFilter(allow= True, regex_string= r"[0-9-]",replacement_string=""))#,col={"sm": 6, "md": 4, "xl": 2})
    genre = ft.TextField(label="жанр", width=500,on_change= validate,input_filter=ft.InputFilter(allow= True, replacement_string="",regex_string=r"[ЁёА-яA-Za-z,.-\s/]+"))#,col={"sm": 6, "md": 4, "xl": 2})
    director = ft.TextField(label="режисёр", width=500,on_change= validate,input_filter=ft.InputFilter(allow= True, replacement_string="",regex_string=r"[ЁёА-я,.-\s/]+"))#,col={"sm": 6, "md": 4, "xl": 2})
    aktors = ft.TextField(label="актёры", width=500,on_change= validate, input_filter=ft.InputFilter(allow= True, replacement_string="",regex_string=r"[ЁёА-я,.-\s/]+"))#,col={"sm": 6, "md": 4, "xl": 2})
    state = ft.TextField(label="состояние", width=500,on_change= validate,
                          input_filter=ft.InputFilter(allow=True, replacement_string="",
                                                      regex_string=r"[ЁёА-я,.-\s/]+"))#,col={"sm": 6, "md": 4, "xl": 2})
    #создание чекбоксов выбора таблиц медиа
    chekbox_film = ft.Checkbox(label="Фильмы", value= False,on_change=checkboxes_changed,adaptive=True)#,disabled= True)
    chekbox_anime = ft.Checkbox(label= "Аниме",value= False,on_change=checkboxes_changed,adaptive= True)#,disabled= True)
    chekbox_mult = ft.Checkbox(label="Мультфильмы", value= False,on_change=checkboxes_changed,adaptive= True)#,disabled= True)
    chekbox_series = ft.Checkbox(label= "Сериалы", value= False,on_change=checkboxes_changed,adaptive=True)#, disabled= True)
    chekbox_mult_series = ft.Checkbox(label="Мультсериал", value= False,on_change=checkboxes_changed,adaptive= True)#, disabled= True)

    #создание кнопок действия
    get_datab = ft.OutlinedButton(text="Получить из БД", width=200, on_click=get_value)#,col={"sm": 6, "md": 4, "xl": 2})#,expand=True)#, disabled=True)
    add_data_button = ft.OutlinedButton(text="Добавить в БД", width=200, on_click=add_value,
                                        disabled=True)  # ,col={"sm": 6, "md": 4, "xl": 2})#,expand=True)
    ...
    panel_checkboxes0 = ft.Row(
        [ft.Row([chekbox_film,
            ...
            chekbox_anime,
            chekbox_mult,
            chekbox_mult_series],),results_panel],spacing=30,)#col={"sm": 6, "md": 4, "xl": 2})#,expand=True)

    panel_textfields = ft.Row( [ft.Column([movie_name,release_date,genre,director,aktors,state]),lv],spacing= 50,auto_scroll=True)#,scroll= True)#,run_spacing={"xs": 10})#,expand=True)
    panel_buttons = ft.Row([add_data_button,get_datab,upd_btn],spacing= 20)#,run_spacing={"xs": 10})#,expand=True)
